...
| `CGC_STATE_DIR` | `/tmp/cgc` | Tool-owned state dir for prompt/refs/answer files — **and, currently, the consult store `control.db`** (threads, rounds, results). Deleting it therefore also deletes consult history and breaks `fire --followup` continuation; prompt/refs/answer files themselves are re-derivable scratch. Not durable across reboots — copy an answer you want to keep (or point `--out`) to your own path. Known issue: the authoritative store belongs in a durable location; until it moves, point `CGC_STATE_DIR` (or `CGC_STORE_DB`) at a durable path if you care about history surviving reboot. |
