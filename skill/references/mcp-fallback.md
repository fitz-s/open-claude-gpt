...
- Confirm the model by script (scanner-safe `javascript_tool`). The current composer splits this in two: a power slider (`Instant / Medium / High / Extra High / Pro`) and a model list (`Latest` / `GPT-5.6 Sol` / `GPT-5.5`) — we want **`Latest` + `Pro`**, and the older `Pro Extended` item no longer exists. Scan the composer controls for both and set whichever is wrong; when the React menus won't drive from JS, this path asks the user to set them by hand. That hand-off means this fallback is **outside the unattended selection guarantee** the CDP backend makes (see `cdp_consult.py`'s "Fully automated — no human step") — it can require a person at the keyboard, so don't report a consult fired over this path as an unattended-confirmed selection. This layout changed at the GPT-6 rollout and can change again; treat a mismatch as a blocker to surface, not something to work around.
