...
**Act on the WHOLE answer — disposition every finding, don't cherry-pick the bugs.** A deep consult's highest-value output is often NOT the defect list but the architecture call, the "here's a design that dominates yours", the "correct-but-suboptimal, ship-anyway" judgment, the reframing of the premise. The failure mode to avoid: grab the easy bug-fixes, mentally nod at the design-level points, and silently drop them. Instead, give **every** substantive finding an explicit disposition — **apply** (do it now), **defer** (right call but not now — record it in a ROADMAP/issue with the reason, so it isn't orphaned), or **reject** (with a concrete reason it doesn't hold). Surface that disposition list to the user. Deferring is legitimate; *disappearing* a suggestion is not. If you find yourself having only fixed bugs after a review that also proposed design changes, you under-used the consult — go back and disposition the rest.
